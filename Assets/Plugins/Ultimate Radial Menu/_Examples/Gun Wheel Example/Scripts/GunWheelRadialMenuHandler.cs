...
public class GunWheelRadialMenuHandler : MonoBehaviour
{
	...
	void Start ()
	{
		...
		for( int i = 0; i < LightWeapons.Length; i++ )
		{
			...
			weaponRadialMenu.RegisterButton( ShowCurrentWeaponInfo, LightWeapons[ i ].radialButtonInfo );
		...
		weaponRadialMenu.OnMenuEnabled += OnRadialMenuEnabled;
		weaponRadialMenu.OnMenuDisabled += OnRadialMenuDisabled;
	...
	void PopulateRadialMenu ( WeaponBase[] weapons, string menuName )
	{
		...
		for( int i = 0; i < weapons.Length; i++ )
			weaponRadialMenu.RegisterButton( ShowCurrentWeaponInfo, weapons[ i ].radialButtonInfo );
