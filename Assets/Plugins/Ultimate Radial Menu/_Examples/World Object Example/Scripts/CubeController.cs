...
public class CubeController : MonoBehaviour
{
	...
	private void Start ()
	{
		for( int i = 0; i < cubeColors.Length; i++ )
		{
			...
			UltimateRadialMenu.RegisterButton( "ObjectExample", UpdateCubeColor, cubeColors[ i ].buttonInfo );
		}
	}
	...
	private void Update ()
	{
		...
		if( mouseButtonDown )
		{
			...
			else if( UltimateRadialMenu.ReturnComponent( "ObjectExample" ).IsEnabled && UltimateRadialMenu.ReturnComponent( "ObjectExample" ).CurrentButtonIndex < 0 )
				UltimateRadialMenu.ReturnComponent( "ObjectExample" ).Disable();
		}
		...
		if( mouseButtonUp )
		{
			...
			if( Physics.Raycast( ray, out hit ) )
			{
				...
				if( hit.transform == onMouseDownTransform )
				{
					...
					UltimateRadialMenu.ReturnComponent( "ObjectExample" ).SetPosition( screenPosition );

					// If the radial menu is currently disabled, then enable the menu.
					if( !UltimateRadialMenu.ReturnComponent( "ObjectExample" ).IsEnabled )
						UltimateRadialMenu.ReturnComponent( "ObjectExample" ).Enable();
				}
			}
		}
	}
